...
| HGR-008 | HUMAN_GATE_3 | MANAGED_PRODUCT_REVISION_PROMOTION | sha256:02ef2d55e8dbd887faf95b8bf891f25278f267d80c7f9d9b5fee26e35f794b2c | product-d4fca9d59fbe7a99279da307e0d8761c-r2 | human operator | GRANTED | Real, live Managed Product Revision History + Restore-as-New Convergence proof against the SAME real, historical Managed Product `product-d4fca9d59fbe7a99279da307e0d8761c` (`golden-work-129`) HGR-007 already proved the promotion gate against. The human operator reviewed: the real restore request ("Bu sürüme geri dön" on Sürüm 1, `product-d4fca9d59fbe7a99279da307e0d8761c-initial`, while Sürüm 2/`...-r2` was current), the real `ready_for_review` checkpoint's own `verification_passed` verdict (0 failures, `base_revision_id` = `...-r2`, `source_basis_resolved` = `...-initial`), and the real live review preview the restore job itself served (`http://127.0.0.1:3000`, confirmed rendering the original "Dashboard / Total Students: 0" content in place of the current "Öğrenci Yönetim Paneli") — before attempting promotion. The FIRST attempt, made before this row existed, was genuinely refused by `promote_modification` (`ARK-ERR-0173`/`ARK-ERR-0167`, real HTTP 403, shown verbatim in the Command Center UI) — proving the restore path is gated through the IDENTICAL, unbroadened `MANAGED_PRODUCT_REVISION_PROMOTION` operation class HGR-007 already used, never `PROMOTE_CHILD_PRODUCT` — and the exact `target`/`revision` values above are copied verbatim from that real refusal message, never guessed. This is a `RUNTIME_OPERATION`-scope grant only; it authorizes promotion of this one exact restore changeset archive against this one exact base revision, nothing else — a different restore, a different base revision, or any other project still requires its own separate grant. |
