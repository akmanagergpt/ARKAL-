...
## Block 9 — Permanent Engineering Reliability mission (ARK-REQ-0397 … 0410)

Like Block 8, these entries are not sourced from MS/BP/VDC text: they register a permanent, standing product mission (**D-031**, `docs/build/DECISION_LOG.md`) — ARKALI's evolution from "an AI that generates code" into an evidence-driven engineering system that observes, diagnoses, repairs, verifies and safely promotes changes for both Managed Products and, under separate Stable Core governance, itself. IDs continue sequentially from the highest previously allocated (`ARK-REQ-0396`). All fourteen entries record `owning_phase: 30` — not because Phase 30's own canonical theme (Golden Product + Golden Repair Verification) covers this mission, but because, per the identical `ARK-REQ-0396`/D-028 precedent immediately above, no existing phase's declared scope does, and Phase 30 is the phase open at the time D-031 ruled. This registration authorizes **no implementation**: `Requirements verified: 0` for this block (Appendix B), and no existing accepted phase's evidence is reopened, amended or rescored.

| ID | Requirement | Source | Class | Phase | Owner | Evidence |
|---|---|---|---|---|---|---|
| ARK-REQ-0397 | ARKALI is a permanent evidence-driven software engineering system capable of observing, diagnosing, repairing, verifying and safely promoting changes for Managed Products and, under separate Stable Core governance, itself — a standing product requirement independent of any session, conversation or handoff text and discoverable by a future agent from this register alone | D-031 | MANDATORY | 30 | control.architecture | doc |
| ARK-REQ-0398 | Model output (reasoning, classification, hypothesis, proposal, repair plan) is never treated as proof of correctness; correctness is established through mechanical evidence (parsing, compilation, static analysis, types, tests, integration/acceptance tests, runtime behavior, logs, database integrity, migration validation, security analysis, dependency analysis, performance measurement, architecture/governance gates, provenance) wherever the applicable dimension exists | D-031 | MANDATORY | 30 | acceptance.engine | doc, integ |
| ARK-REQ-0399 | A shared, evidence-driven self-diagnosis discipline (observe, collect evidence, classify failure, localize root cause, generate hypotheses, mechanically test them) applies uniformly to both Managed Product failures and ARKALI's own failures; no second, divergent diagnosis engine is created for either subject | D-031 | MANDATORY | 30 | engineering.repair | doc, integ |
| ARK-REQ-0400 | Autonomous repair is bounded and evidence-driven, never an unbounded ask-the-model-again loop: failure, repair-plan, patch/changeset and verification-result fingerprints, attempt evidence and a retry budget support repeated-failure and non-progress detection, which stops retrying and escalates to a closed failure-class taxonomy (generated/product code, ARKALI code, checker, test, harness, prompt/convergence, provider/model, dependency, database/schema, migration, data-integrity, concurrency, security, performance, capacity, infrastructure/runtime, architecture/governance, or unknown requiring human investigation) | D-031 | MANDATORY | 30 | engineering.repair | doc, integ, chaos, prop |
| ARK-REQ-0401 | Database correctness (schema validity, migration correctness/ordering/dry-run, backward/forward compatibility, referential/uniqueness/nullability integrity, transaction/isolation/concurrency behavior, query correctness, backup/restore and recovery compatibility) is mechanically evaluable for applicable persistence, and no actor mutates a production database on model belief alone — a potentially destructive change requires mechanical validation, isolated/dry-run evidence and appropriate human authorization | D-031 | MANDATORY | 30 | kernel.persistence | doc, persist, integ, human |
| ARK-REQ-0402 | Changes to a Managed Product or to ARKALI itself are evaluated for security regression (authN/authZ, injection classes, unsafe file/deserialization handling, secret leakage, dependency/supply-chain risk, CORS/session/cookie safety, unsafe redirects, information disclosure, cryptographic misuse, insecure defaults) as a condition independent of functional test success; a passing functional suite never substitutes for this evidence | D-031 | MANDATORY | 30 | control.policy | doc, sec |
| ARK-REQ-0403 | Performance claims about a change rest on real, mechanical performance evidence (profiling, benchmark baselines, latency/throughput/error-rate/resource measurement) rather than inference from unrelated passing tests | D-031 | MANDATORY | 30 | control.capability | doc, integ |
| ARK-REQ-0404 | Capacity or scalability claims rest on real load, stress, soak and concurrency evidence measured against explicit workload and service-level requirements; a smaller proven scale is never extrapolated into a larger unproven one | D-031 | MANDATORY | 30 | control.capability | doc, integ, chaos |
| ARK-REQ-0405 | A runtime failure in production is diagnosed to root cause from real evidence (exception/stack trace, logs, failing request, affected component, process/health/dependency/database/resource state, recent revision or configuration change, reproducibility), never resolved by changing code until the symptom disappears | D-031 | MANDATORY | 30 | engineering.repair | doc, integ |
| ARK-REQ-0406 | A genuine defect fix preserves its generic lesson, where one exists, as a regression test, invariant, checker improvement, validation rule or classified finding; it is never preserved as candidate-specific hardcoding, fixture/domain leakage, a weakened checker, a deleted failing test, or an architecture exception added merely to make a check pass | D-031 | MANDATORY | 30 | acceptance.engine | doc, arch |
| ARK-REQ-0407 | A repair or change is promoted only once the evidence dimensions applicable to it (targeted verification, regression, database, security, performance/capacity, runtime/preview proof, and the governing human/architecture gate) have actually been produced; promotion never rests on an assumed or inferred dimension | D-031 | MANDATORY | 30 | acceptance.engine | doc, integ, human |
| ARK-REQ-0408 | ARKALI prefers honest refusal or escalation over fabricated success: an unknown root cause, an unverifiable repair, a failed security check, an undemonstrated capacity target, or a repair that would require bypassing architecture or governance all resolve to refusal or `STOP_FOR_GOVERNANCE`, never a promoted or claimed result | D-031 | MANDATORY | 30 | control.policy | doc, sec |
| ARK-REQ-0409 | Managed Product repair reuses the existing canonical Product → Revision → isolated Workspace → evidence-driven diagnosis → proposed repair → verification → preview/runtime proof → governed human promotion (where required) → append-only new Revision path; historical accepted revisions remain immutable and a repair never rewrites history | D-031 | MANDATORY | 30 | engineering.repair | doc, integ, prop |
| ARK-REQ-0410 | ARKALI's own self-repair remains governed exclusively by the existing Stable Core mechanisms (`CoreUpgrade`, `HUMAN_GATE_2`, `StableRevisionPointer`); Managed Product repair/promotion authority is never broadened into Stable Core promotion authority, and no actor or mechanism directly mutates the running canonical core, though shared diagnosis/planning/verification primitives may eventually be reused across both | D-031 | MANDATORY | 30 | lifecycle.evolution | doc, arch, sec |

This block registers a permanent mission, not a phase's implementation obligation: unlike `ARK-REQ-0392`–`0396`, none of these fourteen entries is expected to be discharged by a single, already-identifiable future phase — full discharge spans many not-yet-scheduled future phases (self-diagnosis, database/security/performance engineering, and Stable Core self-repair are each substantial future work areas in their own right). Recording them now, all MANDATORY per Rule 3, makes the standing obligation real and mechanically checkable without beginning any of that work: `Requirements verified: 0` for this block (Appendix B), and Phase 30's own real acceptance criteria (five Golden Product families plus unseen-domain generalization, `ARKALI_HANDOFF.md` §9) are unchanged in substance — only its live requirement denominator (`RequirementRegister.for_phase("30")`) grows to include these fourteen, exactly as it already grew by one for `ARK-REQ-0396`. Existing partial primitives are real, cited evidence toward eventual discharge but are explicitly NOT sufficient to mark any entry here SATISFIED: `engineering.repair`'s bounded budget/fingerprint mechanism (`ARK-REQ-0029`, F-0056–F-0059) is a real, working instance of the discipline `ARK-REQ-0400` requires, but only for the narrow Golden Repair corpus, not the general mission; `CandidateWorkspace`/`ArtifactStore`/`JobStore`/`engineering.product_change` (D-030) already implement the literal Product → Revision → Workspace → verify → preview → promote path `ARK-REQ-0409` names, but only for natural-language product edits, not diagnosis-driven repair; `F-0068`/`F-0069`/`F-0070` are real convergence/evidence findings resolved under `ARK-REQ-0028`'s existing evidence-authority principle, illustrating `ARK-REQ-0398` in miniature; no database-health, security-scanning, load-testing, or self-repair-for-ARKALI-itself capability exists anywhere in the repository today.
...
| Metric | Value |
|---|---|
| Total registered requirements | **343 entries** (highest ID allocated: ARK-REQ-0410) |
| MANDATORY | 332 |
| CONDITIONAL | 9 |
| OPTIONAL | 2 |
| Duplicate IDs | 0 |
| Entries with owner assigned | 343 (100%) |
| Entries with evidence definition | 343 (100%) |
