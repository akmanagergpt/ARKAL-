...
**Current state:** **PHASE 7 MACHINE-ACCEPTED. PHASE 8 UNLOCKED — NOT_STARTED.**
...
**Last updated by:** Phase 7 Atomic Package 5 (final integration, fault-injection evidence, traceability, C-17 report and machine acceptance)
...
| 8 | Execution Scheduler + Worker Contract (C-21) | **UNLOCKED — NOT_STARTED** ← current work. Unlocked by Phase 7's machine acceptance, which the gate recorded as progression PERMITTED. Nothing is implemented: `execution.scheduler` holds no module, and Phase 7 asserted its absence rather than assuming it. Scope must be derived from repository authority before any implementation. |
...
**Begin Phase 8 — Execution Scheduler + Worker Contract (C-21).** Phase 8 is unlocked and not started. Nothing is implemented: `execution.scheduler` holds no module, and Phase 7 asserted that absence with controls rather than assuming it.

**Derive the scope from repository authority before writing anything.** Read `REQUIREMENT_REGISTER.md` for the requirements whose Phase column is 8, `CONTRACT_INVENTORY.md` for C-21's row, `IMPLEMENTATION_DEPENDENCY_MATRIX.md` for prerequisites and any human gate, and `ARCHITECTURE.md` §3 for what `execution.scheduler` owns. Do not carry forward any assumption Phase 7 made about what Phase 8 owes — Phase 7 asserted only what scheduling is *not*, which is not the same as knowing what it *is*.

What Phase 7 leaves ready: a durable job that reaches `RESUMING` and waits there for someone to pick it up, `begin_attempt` as the only way to start work, and a persisted execution-ownership model that refuses a stale owner. Phase 8 decides *who* runs a job; Phase 7 already decided *whether* an execution is still alive, and that boundary is enforced by controls that fail on scheduling vocabulary appearing in `execution.durable`.

**Two budget facts to carry in.** `max_orchestration_depth` is at **4 of 4** on `surfaces.command → execution.durable → control.policy → kernel.contracts` — a further hop on that chain breaches it and must be decomposed under ADR-0008, never excepted. `kernel.contracts.errors` fan-in is **14 of 15**.
