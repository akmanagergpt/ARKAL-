...
**No Phase 7 requirement is discharged.** ARK-REQ-0003, 0027, 0059, 0060 and 0061 remain open; there is no Phase 7 report, no traceability record and no gate run. `ARK-REQ-0060` is mechanically **evaluable** and `ARK-REQ-0027` has real integration evidence, and neither is discharge. Cumulative verified stays at **75**. Package 5 is owed.
...
**Last updated by:** Phase 7 Atomic Package 4 (the ARK-REQ-0027 enqueue surface under `surfaces.command`)
...
| 7 | Durable Job + Workflow Core (C-19) | **UNLOCKED — IN PROGRESS, NOT ACCEPTED** ← current work. Packages 1–4 complete: the C-19 durable-job persistence foundation, the durability semantics (attempts, heartbeat/ownership, bounded retries, timeout, cancellation, dead-letter), the job-type registry with pause/resume and the crash-recovery sweep — all under `execution.durable`, migrations `0005_durable_job`, `0006_durable_execution` and `0007_job_type_registry` — and the `ARK-REQ-0027` enqueue surface under `surfaces.command`, which persists durable work and returns a reference without executing it. Package 5 owed. Both prerequisites, Phases 5 and 6, are accepted; the matrix Gate column for Phase 7 is empty, so no human gate applies. *(This cell's prose is the exact wording F-0034 mis-read as accepting the phase; it is written plainly because the parser now reads the declared state and ignores prose — this row is the live proof.)* |
...
**Phase 7 Atomic Package 5 — final integration, evidence, traceability, the C-17 report, and the first Phase 7 acceptance submission.** This is the package that may discharge requirements, and the first in Phase 7 permitted to create acceptance artifacts.

It owes: composed integration evidence across C-19's four packages on real persistence; **real fault-injection evidence** for `ARK-REQ-0059` and `ARK-REQ-0061` at the tier genuinely available — the Phase 4 `ARK-REQ-0121` precedent, which used real fault injection without claiming the future formal corpus. **T12 chaos begins at Phase 31 and `ARK-REQ-0327` must not be claimed.** Then provenance and evidence records, `phase_7_traceability.json`, `phase_7_report.json`, and one run of `python scripts/run_phase_gate.py 7 8`.

Every one of ARK-REQ-0003, 0027, 0059, 0060 and 0061 must be traced to a named implementation and a named evidence source, or reported as not discharged. Phase 8 unlocks only if the gate genuinely earns it.

**Watch the orchestration budget.** `max_orchestration_depth` is at **4 of 4** after Package 4. Another context hop on the `surfaces.command → execution.durable → control.policy → kernel.contracts` chain breaches it; decompose under ADR-0008 rather than requesting a GATE 8 exception.
