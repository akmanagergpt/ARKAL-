...
**No Phase 7 requirement is discharged.** ARK-REQ-0003, 0027, 0059, 0060 and 0061 remain open; there is no Phase 7 report, no traceability record and no gate run. `ARK-REQ-0060` is now mechanically **evaluable**, which is not discharge. Cumulative verified stays at **75**. Packages 4–5 are owed.
...
**Last updated by:** Phase 7 Atomic Package 3 (C-19 job-type registry, pause/resume, crash recovery)
...
| 7 | Durable Job + Workflow Core (C-19) | **UNLOCKED — IN PROGRESS, NOT ACCEPTED** ← current work. Packages 1–3 complete: the C-19 durable-job persistence foundation, the durability semantics (attempts, heartbeat/ownership, bounded retries, timeout, cancellation, dead-letter), and the job-type registry with pause/resume and the crash-recovery sweep, all under `execution.durable`, migrations `0005_durable_job`, `0006_durable_execution` and `0007_job_type_registry`. Packages 4–5 owed. Both prerequisites, Phases 5 and 6, are accepted; the matrix Gate column for Phase 7 is empty, so no human gate applies. *(This cell's prose is the exact wording F-0034 mis-read as accepting the phase; it is written plainly because the parser now reads the declared state and ignores prose — this row is the live proof.)* |
...
**Phase 7 Atomic Package 4 — the enqueue surface for `ARK-REQ-0027`.** Backend only, in `surfaces.command` — API constructs are confined there by structure check 12, and no frontend increment is owed. The HTTP request must **persist and enqueue durable work and return a durable job reference immediately**; `MS §Constitution 8` forbids long AI work inside an HTTP request, so the route submits through C-19 and returns, and a control must prove the request does not execute the job.

Reuse what exists: `JobStore.submit` is already idempotent by the persisted unique constraint, so a repeated submission returns the recorded job rather than creating a second one — the route inherits that rather than restating it. Reuse the injected PEP and the injected clock. Add no scheduler, no worker, no dispatch and no queue arbitration: those are C-21 at Phase 8. `execution.durable → surfaces.command` stays forbidden; the dependency runs the other way.

Still owed after that: **Package 5** integration and fault-injection evidence, provenance, traceability, the C-17 phase report and the Phase 7 acceptance gate.
