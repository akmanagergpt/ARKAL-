...
| 7 | Durable Job + Workflow Core (C-19) | **UNLOCKED — NOT_STARTED** ← next work. Prerequisites are Phases 5 and 6; see their rows above. The matrix Gate column for Phase 7 is empty, so no human gate applies. *(This cell deliberately avoids the token the phase-status parser matches on; see the F-0029 note in `KNOWN_FAILURES.md`.)* |
