...
| Branch | `main` |
| Generated at commit | `c4e0e3b9acc6a0f5f492dbfd03e397f1132c921a` — the `head:` claim in the §12 block. A derived control refuses any other commit in this section, so this row cannot rot the way it did before F-0047 |
...
| Status | **UNLOCKED — IN PROGRESS, NOT ACCEPTED**. `GovernanceState.current_work_phase()` returns `9`. **Packages 1 and 2 of 4 are complete**; Packages 3–4 are not started |
...
**Packages 1 and 2 are complete; Phase 9 is not accepted.**
`control.registry.provider` holds the C-11 record and its parsed authority, and
`ARK-REQ-0053` is now enforced in the source of every declared reference-only
consumer. **No provider runtime, no migration, no Phase 9 acceptance artifact and
no discharged requirement exists.** The `shadow_registry` gap
...
**Begin Phase 9 Atomic Package 3 — ARK-REQ-0219, the never-fabricate control.**
Packages 1 and 2 are complete and committed: the registry owns the seven
concerns, and no reference-only consumer may store, cache, mirror, default,
alias or locally re-derive one. Note Package 3's owner
...
NORMAL. Package 2's own change set derived **PROTECTED_CORE** the same way, from
member `control.architecture`.
...
head: c4e0e3b9acc6a0f5f492dbfd03e397f1132c921a
branch: main
