...
| Branch | `main` |
| Generated at commit | `7b9ddb7936c23d851bdd45b0015f10d975d8d4c7` — the `head:` claim in the §12 block. A derived control refuses any other commit in this section, so this row cannot rot the way it did before F-0047 |
...
| Phase 9B | **UNLOCKED — IN PROGRESS, NOT ACCEPTED** ← current work. Capability Graph Activation. `GovernanceState.current_work_phase()` returns `9B`. Prerequisites **3, 4, 6 and 9** are all MACHINE-ACCEPTED, which is why the dependency machine permits it. Denominator is **3** — `ARK-REQ-0046`, `ARK-REQ-0047`, `ARK-REQ-0048`, none discharged — so cumulative verified stays 83 until 9B is accepted. **Atomic Package 1 of 3 is delivered**: `control.capability` now resolves every external `*_ref` at query time against its owning authority, through injected interfaces, with the reference→authority binding parsed from the canonical `capability_node:` block. `can_perform` is **unchanged** — every capability query still returns `NOT_CONFIGURED` before the activation phase and still refuses to invent a verdict at it, so Phase 8 admission still refuses with `CAPABILITY_NOT_CONFIGURED`. **No phase gate has been run.** Package 2 composes the verdict; Package 3 carries traceability, the C-17 report and the gate |
...
| Architecture violations | **0** (8 gates PASS over **108** real cross-context edges; all **9** numeric budgets measured under ratified contract 1.0.0). Phase 9 Package 1 took this from 100 to 102 (`control.registry.provider → kernel.contracts` via `error_base`); Package 3 took it to 104, `acceptance.engine → control.registry.provider` and `acceptance.engine → control.specification`, both downward and legal. **Phase 9B Package 1 took it to 108**, entirely `control.capability → kernel.contracts` — rank 1 to rank 0, the only direction this context has — and added **no** same-layer edge, because `allow_same_layer: false` forbids one and the referenced authorities are reached by injected interface instead. `acceptance.engine` public surface is **40 of 40** — at its ceiling, which is why `ARK-REQ-0219`'s vocabulary, derivation and check are reached through one public name. `kernel.contracts.errors` fan-in is **15 of 15** and `kernel.contracts.state_machine` fan-in is **15 of 15**. **`max_orchestration_depth` is 4 of 4** — `surfaces.command → execution.durable → control.policy → kernel.contracts` — at its ceiling. Every figure in this row is now re-derived from the gate mechanism by `check_handoff.py` on each run (F-0047); the figures in §4 and §11 are historical records of past commits and are deliberately **not** re-derived. Phase 8 Package 2's 466-line scheduler authority test was decomposed under ADR-0008; no GATE 8 exception was requested |
...
| Capability Graph | **schema, plus query-time reference resolution (Phase 9B Package 1)**. Every `can_perform` query still returns `NOT_CONFIGURED` before the activation phase and still refuses to invent a verdict at it, so nothing yet answers "Can I perform this?" — the verdict composition is Package 2. What exists is the §1 resolution rule: each external `*_ref` is put to its owning authority at query time through an injected `Protocol`, `PASS` is reachable only through a live authority's affirmative answer, and nothing is cached. ADR-0003's split still holds |
...
| Status | **UNLOCKED — IN PROGRESS, NOT ACCEPTED**. `GovernanceState.current_work_phase()` returns `9B`. Phase 9 is MACHINE-ACCEPTED; **Atomic Package 1 of 3 is delivered** and no phase gate has been run |
...
| What activation changes | every capability query currently returns `NOT_CONFIGURED` and production admission returns `CAPABILITY_NOT_CONFIGURED`. 9B is what makes admission able to succeed. ADR-0003 is the authority for the split and must be read, not summarised from here |
...
**Phase 9 is MACHINE-ACCEPTED; 9B is in progress at Package 1 of 3.**
`control.capability` now performs the §1 resolution rule — every external `*_ref`
put to its owning authority at query time through an injected `Protocol`, with
the binding parsed from the canonical `capability_node:` block. **The query
itself is unchanged**: `can_perform` still answers `NOT_CONFIGURED` before the
activation phase and still refuses to invent a verdict at it, so production
admission still refuses with `CAPABILITY_NOT_CONFIGURED`. Composing resolutions
into one determinate verdict is Package 2, and **the canonical set does not state
what verdict an unresolved external reference produces** — that must be derived
there or reported, not guessed.
...
**Continue Phase 9B — Atomic Package 2, the activated query.** Package 1 is
delivered and committed at `7b9ddb7`; **do not begin Package 2 without
authorization**. Package 1 made references resolve. Package 2 must compose a set
of `ReferenceResolution` outcomes into one determinate `CapabilityQueryResult`,
which is what makes the graph able to answer "Can I perform this?" at all.
Derive the requirement set with `RequirementRegister.for_phase("9B")` rather than
assuming anything carries over.

**Derive the verdict mapping; do not guess it, and stop if the canonical set
will not decide it.** `EXECUTION_AND_CAPABILITY.md` §1 states the resolution rule
and `MS §Capability Graph` forbids a query returning "a stubbed, defaulted or
assumed value", but **neither states what verdict an unresolved external
reference produces**. `AUTHORITY_MAP.yaml` `isolation.on_unsatisfiable: DENY` and
`isolation.capability_state_when_unsatisfiable: UNSUPPORTED` are that authority's
data for the isolation condition specifically, not a general rule for a dangling
reference. Package 1 left this open deliberately and recorded it in
`docs/contracts/capability_node.md` §9. If the canonical set does not decide it,
**report the ambiguity before implementing** — the precedent is **D-023**, where
Stage 1 stopped and reported rather than inventing an external-result object.

**Two designed tripwires will fire in Package 2, and neither may be weakened.**
`tests/capability/test_capability_preactivation.py::test_resolution_is_not_implemented_in_the_schema_phase`
and
`tests/execution/test_admission_decision.py::test_the_scheduler_cannot_force_a_configured_verdict`
both assert that `can_perform` raises `PrematureActivation` at the activation
phase. That is Phase 3 refusing to implement a forward phase, and Package 2 is
the phase that owns it. Replace each with a **stronger live control**, exactly as
the Phase 4 durable-surface tripwire was replaced — never by deletion, skip or
xfail.

**What Package 1 already established, so it is not rebuilt.** The
reference→authority binding is parsed from the canonical `capability_node:`
block's own `# -> <context>` annotations; resolution runs through
`ReferenceResolver`, a `Protocol` declared inside `control.capability` because
`allow_same_layer: false` forbids the import and `ARCHITECTURE.md` §4 rule 3
makes interface inversion the answer; a resolver may be asked only
`resolves(reference) -> bool`, so no value crosses; `PASS` is reachable only
through a live authority's affirmative answer, a missing resolver is
`NOT_CONFIGURED` and a refusing one `FAIL`; and nothing is cached, proven by
changing an authority's answer between two calls. Reuse all of it. Do not add a
second graph, a second node model or a second capability authority.

**Read ADR-0003 first.** It is the authority for the schema/activation split and
for the rule Phase 8 consumers already obey: every capability query returns
`NOT_CONFIGURED` before activation, and consumers **must not cache capability
verdicts**. A Phase 8 control counts the authority's calls, so a cache
introduced by activation will be caught.
...
the `shadow_registry` gate reads consumer **source**: a capability node that
stored, cached, mirrored, defaulted, aliased or locally re-derived provider
health, cost, availability, configuration, identity, model identity or fallback
would fail the gate. The graph holds `provider_refs` and resolves them at query
time (ADR-0001).

**Every acceptance report from here on owes an `external_result` declaration on
every recorded run.** The obligation is derived from the register's Phase column
for the never-fabricate requirement Phase 9 discharged — **D-023 in
`DECISION_LOG.md` names it** — so it applies to 9B and every later phase; a
defaulted field is refused.
Read **D-023** in `DECISION_LOG.md`: it records what the canonical set does *not*
define — no external-provider-result object, no request or response identity, no
timestamp binding, no verification flag — and that none of it was invented. Do
not invent it. Declaring `EXTERNAL_PROVIDER_RESULT` will be refused until a later
canonical phase supplies a verifiable binding.
...
head: 7b9ddb7936c23d851bdd45b0015f10d975d8d4c7
branch: main
