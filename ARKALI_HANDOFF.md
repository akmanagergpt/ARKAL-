...
| Branch | `main` |
| Current HEAD | `975b1df566a22240b40e213c6d6eb1dfb12653cc` |
...
backend/                                    pytest suite (1582 passed, 13 skipped)
...
| Phase 7 | **UNLOCKED — IN PROGRESS, NOT ACCEPTED** ← current work. Durable Job + Workflow Core (C-19); matrix prerequisites 5 and 6, Gate column empty. **Packages 1–2 complete**: the C-19 persistence foundation (durable job record, idempotency identity, checkpoint record) and the durability semantics (execution attempts with ownership and heartbeat, bounded retries with row-derived accounting, absolute deadlines, cancellation, dead-letter). Migrations `0005_durable_job` and `0006_durable_execution`. **Packages 3–5 owed.** No requirement discharged, no report, no traceability record, no gate run |
...
| 64 | `975b1df` | **PHASE 7 PACKAGE 4** — the `ARK-REQ-0027` enqueue surface under `surfaces.command`: `POST /api/jobs` persists durable work and returns a reference without executing it. F-0037, F-0038 and F-0039 opened and closed. **Not a phase acceptance** ← HEAD at generation |
...
| Status | **UNLOCKED — IN PROGRESS, NOT ACCEPTED**. Packages 1–2 done, Packages 3–5 owed |
...
| Verification profile | **derive it from the changed paths** with `select_profile`. Package 1 came out PROTECTED_CORE because completing the kernel-error decomposition touched `control.architecture`; Package 2 stayed inside `execution.durable` and came out NORMAL. Never infer it from intention |
...
**Phase 7 Atomic Package 5 — final integration, evidence, traceability, the
C-17 report, and the first Phase 7 acceptance submission.** Do not rebuild
Packages 1–4.

This is the package that may discharge requirements, and the **first in Phase 7
permitted to create acceptance artifacts**. It owes:

- composed integration evidence across C-19's four packages on real persistence;
- **real fault-injection evidence** for `ARK-REQ-0059` and `ARK-REQ-0061` at the
  tier genuinely available. The precedent is Phase 4's `ARK-REQ-0121`: real
  fault injection, without claiming a corpus that does not exist yet. **The
  formal T12 chaos tier begins at Phase 31 and `ARK-REQ-0327` must not be
  claimed**;
- provenance and evidence records;
- `phase_7_traceability.json` and `phase_7_report.json`;
- one run of `python scripts/run_phase_gate.py 7 8`.

Each of ARK-REQ-0003, 0027, 0059, 0060 and 0061 must be traced to a named
implementation and a named evidence source, or reported as **not discharged**.
Check C6 refuses a discharge the traceability record does not support. Phase 8
unlocks only if the gate genuinely earns it.

**Watch `max_orchestration_depth`: it is at 4 of 4.** Another context hop on the
`surfaces.command → execution.durable → control.policy → kernel.contracts` chain
breaches it. Decompose under ADR-0008; do not request a GATE 8 exception.
...
head: 975b1df566a22240b40e213c6d6eb1dfb12653cc
branch: main
