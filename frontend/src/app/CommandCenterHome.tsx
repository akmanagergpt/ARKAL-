...
export function CommandCenterHome({ navigate }: { navigate: (area: AreaId) => void }) {
  return <div className="space-y-6">
    <section className="overflow-hidden rounded-2xl bg-slate-950 px-6 py-8 text-white shadow-sm sm:px-8">
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        ...
        <button className="rounded-lg border border-slate-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800" onClick={() => navigate('workflow')}>Gelişmiş stüdyoyu aç</button>
      </div>
    </section>
